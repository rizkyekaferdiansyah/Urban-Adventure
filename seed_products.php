...
$product = $pdo->prepare('INSERT INTO products (category_id, name, slug, sku, description, price_per_day, stock, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE price_per_day=VALUES(price_per_day), image=VALUES(image), status="active"');
foreach ($products as $item) {
    $name = trim((string) $item['category']);
    $name = str_replace('Lain – Lain', 'Lain-Lain', $name);
    $category->execute([$name, slugify($name)]);
    $categoryId = (int) $pdo->lastInsertId();
    $product->execute([$categoryId, $item['name'], slugify($item['name']), 'UA-' . strtoupper(substr(slugify($item['name']), 0, 3)) . '-' . str_pad((string) $pdo->query('SELECT COALESCE(MAX(id),0)+1 FROM products')->fetchColumn(), 4, '0', STR_PAD_LEFT), 'Perlengkapan outdoor Urban Adventure yang bersih dan terawat.', $item['price'], 5, $item['image']]);
}
$pdo->commit();
